#ifndef IDE_H
#define IDE_H

#include "block_dev.h"

#define SECTSIZE 512  // 默认扇区大小

#define STAT_BSY 0x80   // IDE驱动器忙
#define STAT_DEDY 0x40  // IDE驱动器就绪
#define STAT_DF 0x20    // IDE驱动器错误
#define STAT_ERR 0x01   // 上一次命令失败

#define IDE_CMD_READ 0x20      // IDE读扇区命令
#define IDE_CMD_WRITE 0x30     // IDE写扇区命令
#define IDE_CMD_IDENTIFY 0xEC  // IDE识别命令

// IDE设备端口起始端口定义
#define IOBASE 0x1F0  // 主IDE设备起始操作端口
#define IOCTRL 0x3F4  // 主IDE控制起始控制端口

// IDE设备控制端口偏移量
// 读
#define ISA_DATA 0x00   // IDE数据端口
#define ISA_ERROR 0x01  // IDE错误端口
#define ISA_CTRL 0x02  // IDE控制端口
#define ISA_SECTOR 0x03 // 扇区号
#define ISA_CYL_LO 0x04 // 磁道数低8位
#define ISA_CYL_HI 0x05 // 磁道数高8位
#define ISA_SDH 0x06      // IDE选择端口
#define ISA_STATUS 0x07   // IDE状态端口
// LBA 寻址方式
#define ISA_LBA_LOW 0x03  // 0~7
#define ISA_LBA_MID 0x04  // 8~15
#define ISA_LBA_HIGH 0x05 // 16~23
#define ISA_LBA_TOP 0x06  // 24~27

// 写
#define ISA_PRECOMP 0x01 // 特征
#define ISA_SECCNT 0x02 // 扇区计数
#define ISA_COMMAND 0x07  // IDE命令端口

// IDE设备限制值
#define MAX_NSECS 128              // IDE设备最大操作扇区数
#define MAX_DISK_NSECS 0x10000000  // IDE设备最大扇区号

// IDE设备身份信息在读取的信息块中的偏移
#define IDE_IDENT_SECTORS 20
#define IDE_IDENT_MODEL 54
#define IDE_IDENT_CAPABILITIES 98
#define IDE_IDENT_CMDSETS 164
#define IDE_IDENT_MAX_LBA 120
#define IDE_IDENT_MAX_LBA_EXT 200

#define IDE_DESC_LEN 40  // IDE设备描述信息尺寸

// IDE设备信息
static struct ide_device {
    uint8_t valid;                // 是否可用
    uint32_t sets;                // 命令支持
    uint32_t size;                // 扇区数量
    char desc[IDE_DESC_LEN + 1];  // IDE设备描述
}ide_device;

// 初始化IDE设备
int ide_init(void);

// 检测IDE设备是否可用
bool ide_device_valid(void);

// 获取IDE设备描述
const char *ide_get_desc(void);

// 获得设备默认块数量
int ide_get_nr_block(void);

// 设备操作请求
int ide_request(io_request_t *req);

// 读取IDE设备若干扇区
int ide_read_secs(uint32_t secno, void *dst, uint32_t nsecs);


// 写入IDE设备若干扇区
int ide_write_secs(uint32_t secno, const void *src, uint32_t nsecs);

// IDE设备选项设置
int ide_ioctl(int op, int flag);

// 测试硬盘
void test_ide();

#endif
